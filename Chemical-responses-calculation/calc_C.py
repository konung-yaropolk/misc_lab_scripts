...
# rearrange source csv with only chosen ROIs
# time in the result csv will be zeroed on startpoint
SAVE_SELECTED = True
...
def process_csv(input, stim_A, stim_C, appl_1, wout_1, appl_2, wout_2, appl_3, fibers, last_DRS=None):
    ...
    start_bl_1, end_bl_1 = -CALM_PERIOD+appl_1 / \
        1000, CALM_PERIOD_AFTER_TRIG+appl_1/1000
    start_1, end_1 = SKIP_AFTER_APPLICATION+appl_1/1000,    wout_1/1000

    start_bl_2, end_bl_2 = -CALM_PERIOD+appl_2 / \
        1000, CALM_PERIOD_AFTER_TRIG+appl_2/1000
    start_2, end_2 = SKIP_AFTER_APPLICATION+appl_2/1000,    wout_2/1000

    start_bl_3, end_bl_3 = -CALM_PERIOD+appl_3 / \
        1000, CALM_PERIOD_AFTER_TRIG+appl_3/1000
    ...
    for i, column in enumerate(df.columns[1:], start=1):
        ...
        data_bl_general = df.loc[(
            df[time] >= last_DRS+CALM_PERIOD_AFTER_TRIG) & (df[time] <= end_bl_1), column]
        data_bl_A = df.loc[(df[time] >= start_bl_A) &
                           (df[time] <= end_bl_A), column]
        data_bl_C = df.loc[(df[time] >= start_bl_C) &
                           (df[time] <= end_bl_C), column]
        data_bl_1 = df.loc[(df[time] >= start_bl_1) &
                           (df[time] <= end_bl_1), column]
        data_bl_2 = df.loc[(df[time] >= start_bl_2) &
                           (df[time] <= end_bl_2), column]
        data_bl_3 = df.loc[(df[time] >= start_bl_3) &
                           (df[time] <= end_bl_3), column]
        ...
        resp_1 = peak_amplitude_1 > SIGMAS * \
            std_dev_bl_general and peak_amplitude_1 > 0 and (
                (i in fibers) == YEILD_FIBERS) and stable_baseline
        resp_2 = peak_amplitude_2 > SIGMAS * \
            std_dev_bl_general and peak_amplitude_2 > 0 and (
                (i in fibers) == YEILD_FIBERS) and stable_baseline
        resp_3 = peak_amplitude_3 > SIGMAS * \
            std_dev_bl_general and peak_amplitude_3 > 0 and (
                (i in fibers) == YEILD_FIBERS) and stable_baseline
        ...
        if resp_C and not resp_A and ((i in fibers) == YEILD_FIBERS):
            ...
            if bool_resp_1 and not bool_resp_2 and not bool_resp_3:
                set_1 = 1
            if bool_resp_2 and not bool_resp_1 and not bool_resp_3:
                set_2 = 1
            if bool_resp_3 and not bool_resp_2 and not bool_resp_1:
                set_3 = 1
            if bool_resp_1 and bool_resp_2 and not bool_resp_3:
                intersection_1_2 = 1
            if bool_resp_1 and bool_resp_3 and not bool_resp_2:
                intersection_1_3 = 1
            if bool_resp_2 and bool_resp_3 and not bool_resp_1:
                intersection_2_3 = 1
            if bool_resp_1 and bool_resp_2 and bool_resp_3:
                intersection_1_2_3 = 1
    ...
    # for debug:
        ...
        if SAVE_SELECTED:
            ...
            selected_columns = [df.columns[0]] + [df.columns[i]
                                                  for i in accepted_roi_list]
